...
def main():
  ...
  weird_news='rplusu.json'
  clickbait_news='clickBaitData.json'
  ...
  #Load clickbait news
  fd = open(clickbait_news, 'r')
  for line in fd:
    json_obj = json.loads(line)
    title = ''.join([i if ord(i) < 128 else ' ' for i in json_obj['title']])
    raw_clickbait.append(str(title))
  ...
  print_sentence_structure(raw_clickbait)
  print_num_stop_words(raw_weird)
  print_num_stop_words(raw_clickbait)
  print_avg_word_len(raw_weird)
  print_avg_word_len(raw_clickbait)
  print_pos_distributions(raw_weird)
  print_pos_distributions(raw_clickbait)
  print_quoted_counts(raw_weird)
  print_quoted_counts(raw_clickbait)
  #most_repeated_bigrams(raw_weird)
  #most_repeated_bigrams(raw_clickbait)
  #most_repeated_4grams(raw_weird)
  #most_repeated_4grams(raw_clickbait)
  most_rep_sub_w = most_repeated_subjects(raw_weird)
  most_rep_sub_n = most_repeated_subjects(raw_clickbait)
  print ("Most repeated subjects in weird", most_rep_sub_w)
  print ("Most repeated subjects in clickbait", most_rep_sub_n)
  print_num_articles_with_popular_subject(raw_weird, most_rep_sub_w)
  print_num_articles_with_popular_subject(raw_clickbait, most_rep_sub_n)
  avg_capitalized_words(raw_weird)
  avg_capitalized_words(raw_clickbait)
  avg_animals(raw_weird)
  avg_animals(raw_clickbait)
  avg_body_parts(raw_weird)
  avg_body_parts(raw_clickbait)
  avg_nes(raw_weird)
  avg_nes(raw_clickbait)
  avg_nes_halves(raw_weird)
  avg_nes_halves(raw_clickbait)
  nvn_phrases(raw_weird)
  nvn_phrases(raw_clickbait)
  #'''
  print_len_distributions(raw_weird)
  print_len_distributions(raw_clickbait)
  '''
  ...
  top_clickbait_verbs = get_top_verbs(raw_clickbait)
  print( "Top clickbait verbs =", top_clickbait_verbs)
  #Create the test and training sets
  #shuffle(raw_weird)
  #shuffle(raw_clickbait)
  '''

  train = 12000
  test=3000

  X_raw_train=raw_weird[:train]+raw_clickbait[:train]
  X_raw_test=raw_weird[train:train+test]+raw_clickbait[train:train+test]
  #Create the labels 1st half are weird class 1, rest are clickbait 0
...
#Dense(64, activation="sigmoid", kernel_initializer="uniform", input_dim=922)
  ...
  '''
  print("Try with SVM with RBF kernel")
...
synonyms=['weird', 'supernatural', 'unearthly', 'strange', 'abclickbait', 'unusual',
        'uncanny', 'eerie', 'unnatural', 'unreal', 'ghostly', 'mysterious',
        'magical', 'bizarre', 'peculiar', 'funny', 'curious', 'offbeat',
        'eccentric', 'unconventional', 'unorthodox', 'abclickbait', 'atypical',
